-- =============================================================================
-- LEXBNB PHASE 38 — EXECUTIVE OPEX / CAPEX KPI BREAKDOWN
-- =============================================================================
-- Keep the existing RPC signature and accrual rules, but expose the expense
-- classes needed by the executive homepage. OTA commission is OPEX; CAPEX is
-- reported separately and is deducted only after operating profit.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'operating_expenses', round(v_manual_opex + v_ota,2),
    'capex', round(v_capex,2),
    'total_expenses', round(v_manual_opex + v_ota + v_capex,2),
    'operating_profit', round(v_revenue - v_manual_opex - v_ota,2),
    'net_profit', round(v_revenue - v_manual_opex - v_ota - v_capex,2),
...
BEGIN
  SELECT pg_get_functiondef('public.get_executive_dashboard_snapshot(uuid,text,uuid)'::regprocedure)
    INTO v_definition;
  IF v_definition NOT LIKE '%operating_expenses%' OR v_definition NOT LIKE '%operating_profit%'
     OR v_definition NOT LIKE '%manual_opex%' OR v_definition NOT LIKE '%capex%' THEN
    RAISE EXCEPTION 'PHASE38_EXPENSE_BREAKDOWN_MISSING';
  END IF;
  IF has_function_privilege('anon', 'public.get_executive_dashboard_snapshot(uuid,text,uuid)', 'EXECUTE') THEN
    RAISE EXCEPTION 'PHASE38_ANON_EXECUTE_OPEN';
  END IF;
  IF NOT has_function_privilege('authenticated', 'public.get_executive_dashboard_snapshot(uuid,text,uuid)', 'EXECUTE') THEN
    RAISE EXCEPTION 'PHASE38_AUTHENTICATED_EXECUTE_MISSING';
  END IF;
  RAISE NOTICE 'PHASE 38 OK — executive OPEX/CAPEX breakdown ready';
END;
$$;
