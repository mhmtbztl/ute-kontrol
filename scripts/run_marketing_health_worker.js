...
const { createRepository } = require('../core/marketing_health_worker_repository');
const { runHealthSnapshots } = require('../core/marketing_health_worker_service');
...
async function main() {
  ...
  const result = await runHealthSnapshots(createRepository(client), {
    tenantId: process.env.MARKETING_TENANT_ID || null,
    asOf: new Date().toISOString(),
    limit: Number(process.env.MARKETING_HEALTH_LIMIT || 200)
  });
